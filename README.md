...
- PyTorch ranking baseline (`backend/ml/`) — `crops_v1` model inputs (32×32 median-subtracted temporal crop stacks per track, `python -m ml.crops --sequence-id N`), split-aware `TrackCropDataset`, CNN+GRU `temporal_ranker_v1`, seeded training with best-val-AP checkpointing and ranking metrics (`python -m ml.train`; test split only via `--evaluate-test`). First run: held-out comet ranked 24 of 2,312 tracks.
